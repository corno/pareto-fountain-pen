...
export const Block_Part: t_signatures.Block_Part = ($) => _p.deprecated_cc($, ($): t_out.Block_Part => {
    switch ($[0]) {
        ...
        case 'rich list': return _p.ss($, ($) => ['rich list', ({
...
export const Group_Part: t_signatures.Group_Part = ($) => _p.deprecated_cc($, ($): t_out.Group_Part => {
    switch ($[0]) {
...
export const Node: t_signatures.Node = ($) => _p.deprecated_cc($, ($): t_out.Node => {
    switch ($[0]) {
...
export const Directory: t_signatures.Directory = ($) => $.__d_map(($) => Node(
    $
))
