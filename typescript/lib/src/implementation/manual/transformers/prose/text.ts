...
export const Paragraph: p_ti.Transformer_With_Parameter<d_in.Paragraph, d_out.Text, d_x.Parameters> = ($, $p) => p_text_from_list(
    t_to_list_of_characters.Paragraph($, $p),
    ($) => $
)

export const Phrase: p_ti.Transformer_With_Parameter<d_in.Phrase, d_out.Text, d_x.Parameters> = ($, $p) => p_text_from_list(
